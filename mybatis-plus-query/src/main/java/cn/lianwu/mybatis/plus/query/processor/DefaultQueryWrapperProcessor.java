...
import cn.lianwu.mybatis.plus.query.QueryConstants;
...
public class DefaultQueryWrapperProcessor extends BaseQueryWrapperProcessor {
     ...
     * <strong>若使用or进行条件设置，为确保查询结果的正确性，建议为这些or查询条件声明一个单独的分组。</strong>
     * <strong>后置处理器只针对查询注解中声明的分组有效，意味着即使补充了某个分组的后置处理逻辑，若该分组未在注解中声明，则相应的后置处理逻辑也不会被执行</strong>
     * </pre>
     *
            ...
            if (existsAnyValidCondition.get()) {
                ...
                wrapper.and(w -> {
                    if (!emptyValidStatements) {
                        ...
                        // 否则表明该分组下只存在后置处理器，则补充“1 = 1”确保QueryWrapper构建的正确性
                        w.apply("1 = 1");
                    }
                    // 若存在后置处理器则补充查询条件设置
